...
const runBot = async () => {
    ...
    if(amountOut[0].lt(amountOut[token.length])) {
        ...
        //     let next = (i + 1) % token.length;
        //     un2AmountOut[i + 1].gt(suAmountOut[i + 1])
        //     ? await swap(i, amountOut[i], next, un2AmountOut[i + 1], un2Router)
        //     : await swap(i, amountOut[i], next, suAmountOut[i + 1], suRouter);
        // }
    }
    console.log();
...
const inputMax = async () =>{
    let maxProfit = 0, nowProfit = 0, lastProfit;
    let maxCount = 0, nowCount = 1, lastCount;
    let step = 10, threshold = 0.00001;
    let isFound = false, isForward = true;
    while(1) {
        nowProfit = runBot(nowCount);
        if(nowProfit >= maxProfit) {
            maxProfit = nowProfit;
            maxCount = nowCount;
            if(!isFound) {
                nowCount = maxCount * step;
            } else {
                if(isForward) {
                    nowCount = (maxCount + lastCount) / 2;
                } else {
                    nowCount = (maxCount + lastCount) / 2;
                }
            }
