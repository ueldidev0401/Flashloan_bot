...
const calculateProfit = async (amountIn, tokenPath) => {
    ...
    for(let i = 0; i < tokenPath.length; i++) {
        let next = (i + 1) % tokenPath.length;
            res = await getPriceFrom1InchApi(
            tokenPath[i].address,
        ...
        // console.log(res);
        let toAmountPrint = toPrintable(res.toTokenAmount, tokenPath[next].decimals, fixed);
        let dexName = res.protocols[0][0][0].name;
        console.info(tokenPath[next].symbol + "--->" + toAmountPrint);
        console.info("protocols: " + dexName);
        console.info("estimatedGas: ", res.estimatedGas);
        ...
        table.addRow({
            ...
            'UniSwapV3': dexName == 'UNISWAP_V3'?`${toAmountPrint} ${tokenPath[next].symbol}`: '',
            'UniSwapV2': dexName == 'UNISWAP_V2'?`${toAmountPrint} ${tokenPath[next].symbol}`: '',
            'SushiSwap': dexName == 'SUSHI'?`${toAmountPrint} ${tokenPath[next].symbol}`: '',
            'ShibaSwap': dexName == 'SHIBA'?`${toAmountPrint} ${tokenPath[next].symbol}`: ''
        });
    }
